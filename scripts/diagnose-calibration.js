...
async function main() {
  ...
  try {
    ...
    const records = lines.map((line, index) => {
      try {
        ...
        throw new Error(`Invalid JSON on gold line ${index + 1}: ${error.message}`);
      }
    });
    report.gold_file = {
      exists: true,
      records: records.length,
      first_id: records[0]?.gold_evaluation_id || records[0]?.id || null,
      last_id: records.at(-1)?.gold_evaluation_id || records.at(-1)?.id || null
    };
  ...
  try {
    const migration = await db.query(
      ...
      ["006_calibration_attempts.sql"]
    );
    report.checks.push({
      name: "migration_006",
      status: migration.rowCount ? "ok" : "failed",
      message: migration.rowCount ? "Migration 006 is recorded as applied" : "Migration 006 is not recorded as applied"
    });
  } catch (error) {
  ...
  try {
    const table = await db.query(`
      ...
      WHERE table_schema = 'public' AND table_name = 'calibration_attempts'
      ORDER BY ordinal_position
    `);
    report.calibration_table = {
      exists: table.rowCount > 0,
      columns: table.rows
    };
    ...
    report.checks.push({
      name: "calibration_attempts_schema",
      status: missing.length ? "failed" : "ok",
      message: missing.length ? `Missing columns: ${missing.join(", ")}` : "Required calibration_attempts columns are present"
    });
  } catch (error) {
  ...
  try {
    const attempts = await db.query(
      "SELECT COUNT(*)::int AS count FROM calibration_attempts WHERE reviewer_id = $1",
      ["reviewer01"]
    );
